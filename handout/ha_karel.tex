...
\title{\protect\parbox{\textwidth}{\protect\centering Mastering the game of Go \\ with deep neural networks and tree search}}}
...
\begin{description}
  \item [Training] is the phase, when the model ``learns'', i.e. it optimizes the~error function by adjusting inner parameters.
    The portion of~data set used for~training is called the~\emph{training set}.
  \item [Testing] is the phase evaluating, how well the model can make predictions about unseen data.
    The portion of~data set used for~testing is called the~\emph{testing set}.
  ...
  \item [Gradient descent] (GD) is an~iterative optimization algorithm to find a~(generally local) minimum of~a~cost or an~error function.
  ...
  \item [Stochastic gradient descent] (SGD) is a~variant of~GD, which uses only one sample of~the training set per~iteration to~update the parameters.
    Rather than using the~overall error of~the~training set, it works with an~error of~a~single sample.
  \item [Overfitting] is a~situation where the model learns the training data by~heart, instead of~learning the big picture.
    This happens when the model is too complex for~the size of~the training data, e.g. using a~$100$-degree polynomial as a~model to fit 100 data points.
...
\begin{description}
  \item [Self-play] means learning from game instances played by~the agent against itself (or in~the case of~AlphaGo, against its previous versions).
...
\begin{description}
  \item [Monte Carlo tree search] (MCTS) is a~Monte Carlo heuristic of~the classical tree search.
    Instead of traversing the entire game tree, however, the MCTS selects the~most promising moves, expanding the search tree based on~random sampling.
...
Inspired by biological neural networks, artificial neural networks (ANN) are a network of interconnected nodes that make up a model.
They can be defined as statistical learning models that are used to approximate functions that depend on a large number of inputs.
...
\begin{description}
  \item [Convolutional neural network] (CNN) is a~neural network suitable for high-dimensional inputs (e.g. many pixels in an~image).
    CNNs are typically used in~computer vision (identifying objects in an~image, face detection in~photos etc.).
  ...
  \item [Deep neural network] (DNN) is a~neural network with many hidden layers.
    It can model complex non-linear relationships, e.g. in~speech, images, videos or board positions of~Go.
\end{description}
...
\begin{theorem}
  The (distributed) AlphaGo program has reached the \underline{super-human} level of~expertise in~the game of~Go.
\end{theorem}

\begin{proof}
  The proof is left as an~exercise to the reader.
  The exercise is to make an~effort to follow the slides.
  :-)
\end{proof}
